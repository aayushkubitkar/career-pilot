...
- **`/scrape` was deliberately lenient** — its `match_score` (when present in the candidate row) is a title-relevance hint, not a fit signal. A low `match_score` on a job that survived to `new` status means "the title didn't contain the search words" (a *Transaction Risk Decisioning* PM for a "fraud" query), which is exactly what this deep read resolves. Score every candidate on the fetched JD regardless of `match_score`; do not let a low one pre-empt the analysis or lower a score.
- **Level / seniority is scored here**, not filtered at `/scrape`. Apply `04-job-evaluation.md`'s level check: a title at Staff / Principal / Group / Director / VP against a PM/Senior-PM target is over-target — cap Career Alignment and add a "⚠ over-target level" flag, don't silently drop it.
